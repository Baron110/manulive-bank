                ...
                <Fade in={true} timeout={1000}>
                    <StyledPaper elevation={0}>
                        ...
                        <form onSubmit={handleSubmit}>
                            <TextField fullWidth label="Email Address" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required sx={{ mb: 2 }} InputProps={{ startAdornment: <InputAdornment position="start"><Email sx={{ color: '#0A1E3F' }} /></InputAdornment> }} />
                            <TextField fullWidth label="Password" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required sx={{ mb: 2 }} InputProps={{ startAdornment: <InputAdornment position="start"><LockOutlined sx={{ color: '#0A1E3F' }} /></InputAdornment>, endAdornment: <InputAdornment position="end"><IconButton onClick={() => setShowPassword(!showPassword)} edge="end">{showPassword ? <VisibilityOff /> : <Visibility />}</IconButton></InputAdornment> }} />
                            
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
                                <FormControlLabel control={<Checkbox checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} sx={{ color: '#0A1E3F' }} />} label="Remember me" />
                                <Link href="#" sx={{ color: '#0A1E3F', textDecoration: 'none' }}>Forgot password?</Link>
                            </Box>

                            <GradientButton fullWidth type="submit" disabled={loading} endIcon={<ArrowForward />}>{loading ? 'Signing in...' : 'Sign In'}</GradientButton>
                            ...
                            <Box sx={{ textAlign: 'center', mt: 3 }}>
                                <Typography variant="body2" color="text.secondary">
                                    ...
                                    <Link href="#" onClick={(e) => { e.preventDefault(); onSwitchToSignUp(); }} sx={{ color: '#0A1E3F', fontWeight: 600 }}>Create Account</Link>
                                </Typography>
                            </Box>
